...
| **03** website<br>*in progress* | Homepage restructured to the buyer's journey, sections extracted into components, analytics and the SEO/legal gaps closed. SEO researched again against the live site and current crawler behaviour: robots, sitemap, per-route heads and canonicals confirmed shipped; client-side rendering identified as the binding constraint; Google Business Profile found to be blocked on the missing trading address. **14 Sep** | **Prerender the four public routes.** The live home page serves 29 bytes of `<body>` and zero readable text: Google renders it, GPTBot, ClaudeBot and PerplexityBot do not, so the site is invisible to every assistant. `build-routes.mjs` already writes a shell per route and needs only to inject the body. **No dependency** — hours of work, see `seo-strategy-2026-09.md`. Then the illustrative examples still want a real case study, which **waits on `17-proof`** and a first delivery. |
