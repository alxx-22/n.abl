...
| **03** website<br>*in progress* | Homepage restructured to the buyer's journey, sections extracted into components, analytics and the SEO/legal gaps closed. SEO baselined, then fixed. The home page served crawlers 29 bytes of `<body>` and zero readable text; the four public routes are now rendered at build time and the home page carries 7,844 characters. Trailing-slash regression from the Cloudflare move fixed, JSON-LD entity added, `check-seo.mjs` asserts all 29 invariants as the last step of the build. Not yet deployed. **14 Sep** | **Deploy, then verify Search Console.** The prerendering is committed and the build passes, but nothing is live until `npx wrangler deploy` runs; then `npm run test:seo:live`. After that, verify the property in Search Console by DNS TXT — there is still no impressions data at all, which makes every claim about search performance inference. Both are in `seo-handover-2026-09-14.md` §3. **No dependency.** The illustrative examples still want a real case study, which **waits on `17-proof`** and a first delivery. |
...
| **11** outreach<br>*not started* | Two human approval gates written, both before anything leaves the building. First-contact email template written, mapping every element to the rule requiring it. Personalisation settled: observations come from the registers first, a hook library second, a model only for the residual; "personalised" means the observation is specific, never that the message carries a name. Gemini staging, Workers AI and n8n considered and recorded with reasons. The writer then moved into Supabase and is live: pg_cron wakes an edge function every ten minutes, it reads three leads, writes each one its own observation through two staged models and stores the evidence. Runs with the PC off; stops at the human gate and sends nothing. **14 Sep** | **Add `GEMINI_API_KEY` as a Supabase edge function secret** and the writer starts on its own — see `supabase-writer.md` §1. Then read what it wrote and put the prompts in Alex's voice, which is the one part that cannot be delegated. **No dependency.** The postal assessment and the role-addressed letter remain the blocker for *sending* anything, which is unchanged: the writer stops at the human gate. |
