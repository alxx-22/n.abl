...
| **11** outreach<br>*not started* | Two human approval gates written, both before anything leaves the building. First-contact email template written, mapping every element to the rule requiring it. Personalisation settled: observations come from the registers first, a hook library second, a model only for the residual; "personalised" means the observation is specific, never that the message carries a name. Gemini staging, Workers AI and n8n considered and recorded with reasons. **13 Sep** | **Write `hooks.md` and wire the five registers into `observe.mjs`.** `merge.mjs` already carries CQC, FSA, ICO and charity fields per lead and `observe.mjs` reads none of them, which is why 68 of 77 August drafts shared one observation. Registers before page scraping, decided in `personalisation-and-hooks.md`. **No dependency** — no key, no provider, and it does not wait on the postal assessment. That assessment and the role-addressed letter remain the blocker for sending anything. |
