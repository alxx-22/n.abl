...
| **07** crm<br>*in progress* | Pipeline built and deployed; `compliance-schema.md` specifies the missing fields as DDL. Given an entry point in the team space and moved from `/sales-intelligence` to `/crm`. **16 Aug** | **Apply the compliance migration.** Technically depends on nothing — write the SQL and run it. It gates `10` and `11` entirely, so it is the highest-leverage item on this page. |
