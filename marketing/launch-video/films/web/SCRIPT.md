...
generated by the scripts in this folder, except the hand drums, toms and
shaker in the music, which are played from the GeneralUser GS SoundFont
(free for commercial music). No stock footage, stock music or licensed voice
is involved. See
...
the AI film's 124, on percussion rather than an EDM build.
...
- **Music.** Percussion, and almost nothing else, at 144 BPM:
  - a punchy kick, wide layered claps with a finger snap and a snare under
    them on two and four;
  - crisp sixteenth hats with a triplet roll every other bar, open hats on
    the offbeats in the second half;
  - congas and bongos in a syncopated pattern, a shaker, rim clicks, and tom
    fills into each four-bar phrase;
  - one tonal sound: a deep 808 boom on B that marks the bars and the big
    moments. No chords, no melody, no vocals.

  Under the grey wall it is just a heartbeat kick, a rim and a shaker. A
  snare and tom run leads into a beat of air and the drop on "work". The
  groove goes four to the floor, with a floor tom driving it, for the web
  apps and the devices. Before the logo it strips back to accelerating
  claps and a tom roll, lands on the dot and ends on one hit.
