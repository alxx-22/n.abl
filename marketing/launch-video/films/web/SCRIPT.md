...
generated by the scripts in this folder, except the Rhodes in the music,
which is played from the GeneralUser GS SoundFont (free for commercial
music). No stock footage, stock music or licensed voice
is involved. See
...
the AI film's 124, on warm French-touch electronica rather than an EDM build.
...
- **Music.** Warm French-touch electronica at 144 BPM in B minor, drum-led,
  after the reference "Hirā" by Aerds (a French producer tagged french touch
  and electronica, whose records are made on analog synths and a Fender
  Rhodes):
  - clean, punchy drums: kick on every beat, claps and snare on two and
    four, an open hat on every offbeat with closed sixteenths around it,
    clap runs into each four-bar phrase. No hand drums, no toms;
  - a Moog-style bass pulsing on the offbeats, with sixteenth pick-ups in
    the second half;
  - Fender Rhodes chords (Bm9, Gmaj7, Dmaj7, A add9) from the SoundFont, on
    the one and pushed after it;
  - a filtered analog arpeggio with a dotted-eighth echo as the hook,
    opening over each phrase and brighter in the second half;
  - a soft two-oscillator pad underneath.

  The opening loop plays behind a closed filter, and the filter opens as
  the line turns to "Yours", into the drop on "work". Before the logo it
  breaks down to the pad, the Rhodes and the arpeggio with claps speeding
  up, lands on the dot and ends on a ringing Bm9.
- **Effects.** A tick for *0 enquiries*, the dive into "Yours", a riser,
  ...
  dot lands.
- **Mix.** The music sits at a steady level: it dips only 3.5 dB under the
  voice, slowly, so it never jumps up between lines (under the voice and
  between lines differ by under 1 dB). The biggest effects are trimmed for
  this film (`SFX_TRIM` in `film.py`) and peak well under the voice. Master
