...
async function run() {
  ...
  /* ---------------- ROUTES ---------------- */
  {
    ...
    /* The site answers visitors with a language model and the panel carries no
       disclaimer of its own, so the disclosure lives here or nowhere. Each
       document has a different job: privacy says what is sent and to whom,
       terms says the answers do not bind us, cookies says nothing is kept. */
    for (const [path, must] of [
      ['/privacy', [/generated by an AI language model/i, /not written by a person/i,
        /Groq, acting as our processor/i, /United States/i]],
      ['/terms', [/AI assistant/i, /not an offer/i, /not.{0,20}reviewed by a person/i]],
      ['/cookies', [/AI assistant/i, /not kept by us/i]],
    ]) {
      await page.goto(`${BASE}${path}`, { waitUntil: 'networkidle' })
      await page.waitForTimeout(700)
      const t = await page.evaluate(() => document.body.innerText)
      for (const re of must) {
        check(`${path} discloses AI use: ${re}`, re.test(t))
      }
    }

    /* The site left Netlify in August 2026. A policy naming the wrong host is
       wrong about who processes a visitor's request. */
    for (const path of ['/privacy', '/terms', '/cookies']) {
      await page.goto(`${BASE}${path}`, { waitUntil: 'networkidle' })
      await page.waitForTimeout(500)
      const t = await page.evaluate(() => document.body.innerText)
      const stale = /(is served by|hosted by|hosts this site)[^.]*Netlify/i.test(t)
      check(`${path} does not name Netlify as the current host`, !stale)
    }
    await page.close()
  }
