...
/* ------------------------------------------------------------------
   ...
   ------------------------------------------------------------------ */
/* Any canvas cut defines CANVAS_HTML, and optionally CANVAS_SCENE
   naming one of the site's scenes to mount inside it. */
const HAS_CANVAS = typeof CANVAS_HTML !== 'undefined'
let canvasScene = null

if (HAS_CANVAS) {
  canvasEl.innerHTML = CANVAS_HTML
  /* The scene on the canvas is the one the site ships, at canvas size. */
  const host = document.getElementById('cvScene')
  const named = typeof CANVAS_SCENE !== 'undefined' ? SCENES[CANVAS_SCENE] : null
  if (host && named) {
    host.innerHTML = named.make()
    const svg = host.querySelector('svg')
    svg.setAttribute('viewBox', SCENE_VIEWBOX)
    svg.setAttribute('width', '100%')
    svg.setAttribute('height', '100%')
    svg.setAttribute('preserveAspectRatio', 'xMidYMid meet')
    canvasScene = { scene: named, els: named.bind(host) }
  }
}
...
/* ------------------------------------------------------------------
   ...
   A camera moving over a dead page is a slideshow with a tripod. Every
   cell is prepared once — type split into per-word masks, bars and
   rules told their target size — and then plays against its own clock
   the first time the camera arrives on it.

   Same grammar as the other cuts: words ride out of a mask on a stagger
   with a little overshoot, bars and rules wipe rather than fade, and
   nothing is ever completely still.
   ------------------------------------------------------------------ */
const CELLS = new Map()

function prepareCells() {
  for (const el of canvasEl.querySelectorAll('[data-cell]')) {
    const movers = []

    for (const line of el.querySelectorAll('.cvKick, .cvHuge, .cvLine, .cvPaneK, .cvFoot')) {
      const words = String(line.textContent).trim().split(/\s+/)
      line.replaceChildren(...words.map((w) => {
        const mask = document.createElement('span')
        mask.className = 'cvW'
        const inner = document.createElement('i')
        inner.textContent = w
        mask.appendChild(inner)
        movers.push({ kind: 'word', el: inner })
        return mask
      }))
    }

    for (const bar of el.querySelectorAll('.cvBar')) {
      movers.push({ kind: 'bar', el: bar, w: bar.style.width || '60%' })
      bar.style.width = '0%'
    }
    /* Error cells light one after another as the camera pulls back, so
       the count is something you watch happen rather than read. */
    for (const bad of el.querySelectorAll('.dCell--bad')) movers.push({ kind: 'err', el: bad })
    for (const d of el.querySelectorAll('.cvDown')) movers.push({ kind: 'draw', el: d })
    for (const c of el.querySelectorAll('.cvCaret')) movers.push({ kind: 'caret', el: c })
    for (const pane of el.querySelectorAll('.cvPane, .cvMark')) movers.push({ kind: 'pane', el: pane })

    CELLS.set(el.dataset.cell, { el, movers, seen: -1, lit: -1 })
  }
}

/* Called here rather than from the mount block above, which sits before
   `CELLS` is declared — hoisted, but in the temporal dead zone. */
if (HAS_CANVAS) prepareCells()

/* One clock per cell, started when the camera first arrives, so a cell
   held across two sections does not restart its type halfway through. */
function drawCell(name, time, sectionStart, reveal) {
  const c = CELLS.get(name)
  if (!c) return
  if (c.seen < 0 || sectionStart < c.seen) c.seen = sectionStart
  const lt = time - c.seen

  /* A second clock for anything that must wait for a later beat. The
     sheet is on screen from the first frame, so its errors would
     otherwise finish lighting two seconds in — long before the camera
     has pulled back far enough for anyone to see them. */
  if (reveal && (c.lit < 0 || sectionStart < c.lit)) c.lit = sectionStart
  const litT = c.lit < 0 ? -99 : time - c.lit

  c.movers.forEach((m, i) => {
    const at = 0.18 + i * 0.05
    const raw = seg(lt, at, at + 0.62, LINEAR)
    const q = backOut(raw, 1.15)
    const held = Math.min(raw, 1)
    const ph = i * 1.4

    if (m.kind === 'word') {
      m.el.style.transform =
        `translateY(${((1 - q) * 112).toFixed(2)}%) ` +
        `rotate(${(Math.sin(time * 0.5 + ph) * 0.45 * held).toFixed(2)}deg)`
    } else if (m.kind === 'bar') {
      m.el.style.width = `calc(${m.w} * ${Math.min(q, 1).toFixed(3)})`
    } else if (m.kind === 'draw') {
      m.el.style.transformOrigin = '50% 0'
      m.el.style.transform = `scaleY(${Math.min(q, 1).toFixed(3)})`
    } else if (m.kind === 'err') {
      /* On its own clock, and staggered wide enough that they land one
         at a time across the pull-back rather than all together. */
      m.el.style.opacity = seg(litT, 0.25 + i * 0.055, 0.25 + i * 0.055 + 0.5, EASE_OUT).toFixed(3)
    } else if (m.kind === 'caret') {
      m.el.style.opacity = (raw >= 1 && Math.floor((lt - at) * 2) % 2 === 0 ? 1 : 0).toFixed(2)
    } else if (m.kind === 'pane') {
      m.el.style.opacity = held.toFixed(3)
      m.el.style.transform =
        `translateY(${(lerp(34, 0, q) + Math.sin(time * 0.42 + ph) * 6 * held).toFixed(1)}px)`
    }
  })
}

function drawCamera(lt, s, prev, time) {
  ...
  if (s.cell) drawCell(s.cell, time, s.t0, s.reveal)
}
...
function draw(time) {
  ...
  /* ---- the camera ---- */
  if (HAS_CANVAS) drawCamera(lt, s, prev, time)
...
window.reel = { seek, play, pause, draw, SHOW, TOTAL,
...
/* The show's own default first, then the URL, which wins. */
if (typeof SHOW_SPEED !== 'undefined') SPEED = SHOW_SPEED
const wanted = parseFloat(q.get('speed'))
if (Number.isFinite(wanted) && wanted > 0.05 && wanted <= 4) SPEED = wanted

/* Reflect it in the control rather than leaving 1x lit while it runs at a half. */
