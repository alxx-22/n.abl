...
function drawCard(lt, s, prev) {
  ...
  /* 4.5% of the card, whatever its size. On a 540px card that is 24px
     of visible overshoot rather than 5. */
  const swell = 1 + 0.045 * over
  card.style.width = `${(lerp(a.w, b.w, travel) * swell).toFixed(1)}px`
  card.style.height = `${(lerp(a.h, b.h, travel) * swell).toFixed(1)}px`
  card.style.borderRadius = `${lerp(a.r, b.r, travel).toFixed(1)}px`
  ...
  /* No filter: blur() anywhere near this. Blurring a card-sized subtree
     ...
     and it took the renderer down roughly where the card sections start.
     Directional motion blur belongs in the edit. */
  ...
  for (const [name, el] of Object.entries(faces)) {
    ...
    /* Once the edge has passed, the outgoing face is switched off rather
       than left fully clipped — a clipped element is still composited. */
    ...
    /* The incoming content flies in from the right, overshoots its mark
       by about 15px and corrects. The outgoing content is pushed left as
       it goes, so the two are never simply sitting on top of each other. */
    el.style.transform = incoming
      ? `translateX(${lerp(150, 0, slide).toFixed(1)}px) scale(${(1 + 0.03 * over).toFixed(4)})`
      : `translateX(${(-46 * travel).toFixed(1)}px)`
  }
