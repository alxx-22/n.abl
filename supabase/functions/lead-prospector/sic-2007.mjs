/* SIC 2007 — code to description, as Companies House publishes it.

   Vendored rather than fetched at run time. The list changes once a decade
   and a lead pull should not fail because a reference page was slow.

   Source:  https://resources.companieshouse.gov.uk/sic/
   Fetched: 2026-09-22, 731 codes, parsed from the published table and
            spot-checked against eight codes known independently.

   WHY THIS EXISTS. The bulk Companies House file gives SIC as
   "43210 - Electrical installation". The REST API gives it as a bare
   "43210". Triage copes either way — it strips to the digits — but
   promote.mjs takes the text after the dash as the lead's `industry`, and
   the outreach writer turns that into "Companies House records their
   activity as: {industry}". Without this table a lead pulled through the
   API would have been written to as a business whose activity is 43210.
*/

export const SIC_2007 = Object.freeze({
